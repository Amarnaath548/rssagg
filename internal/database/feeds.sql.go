...
const getNextFeedsToFeatch = `-- name: GetNextFeedsToFeatch :many
SELECT id, created_at, updated_at, name, url, user_id, last_fetched_at FROM feeds
ORDER BY last_fetched_at ASC NULLS FIRST LIMIT $1
`

func (q *Queries) GetNextFeedsToFeatch(ctx context.Context, limit int32) ([]Feed, error) {
	rows, err := q.db.Query(ctx, getNextFeedsToFeatch, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
